...
*Setup instructions for a Gitian build of Tridentlayers using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Tridentlayers
...
- [Building Tridentlayers](#building-tridentlayers)
...
Clone the git repositories for tridentlayers and Gitian and then checkout the tridentlayers version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the tridentlayers repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for Gitian to work offline.

Building Tridentlayers
----------------

To build Tridentlayers (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the tridentlayers repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tridentlayers/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/tridentlayers-project/tridentlayers
...
For example:
```bash
URL=https://github.com/user/tridentlayers.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit tridentlayers=${COMMIT} --url tridentlayers=${URL} ../tridentlayers/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tridentlayers=${COMMIT} --url tridentlayers=${URL} ../tridentlayers/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tridentlayers=${COMMIT} --url tridentlayers=${URL} ../tridentlayers/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tridentlayers-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tridentlayers-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/tridentlayers-build.assert
```
...
[tridentlayers/gitian.sigs](https://github.com/tridentlayers-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
